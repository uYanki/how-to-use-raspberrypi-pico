...
spi_inst_t* st7789_spi = LCD_SPI_PORT;

uint16_t st7789_pinCS  = LCD_PIN_CS;
uint16_t st7789_pinDC  = LCD_PIN_DC;
int16_t  st7789_pinRST = -1;

uint16_t st7789_pinSCK = LCD_PIN_CLK;
uint16_t st7789_pinTX  = LCD_PIN_DIN;
...
void initSPI()
{
    spi_init(st7789_spi, 1000 * 1000);
    spi_set_format(st7789_spi, 16, 1, 0, SPI_MSB_FIRST);
...
{
    ST7789_RegCommand();
    spi_set_format(st7789_spi, 8, 1, 0, SPI_MSB_FIRST);
...
void ST7789_SendCommand(uint8_t commandByte, uint8_t* dataBytes, uint8_t numDataBytes)
{
    ST7789_Select();
...
int main()
{
    ...
    for (uint8_t row = 0; row < 240; ++row) {
        for (uint8_t col = 0; col < 135; ++col) {
            LCD_WritePixel(row, col, 0xF800);
        }
    }
